...
diagonalize start m = last $ m : unfoldr (uncurry go) (max 1 (min dim start), m)
  where
    dim = M.nrows m
    go n mat = do
...
firstNonzero m t = amap go [t .. M.ncols m]
  where
    go jt = do
...
elimColumnAtPivot (Pivot (t, jt)) mat = lastMaybe $ unfoldr go (mat, nonzeros mat startIndices)
  where
...
smithNormalForm mat = last $ m0 : unfoldr go m0
  where
